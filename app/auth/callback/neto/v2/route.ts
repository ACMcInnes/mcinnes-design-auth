...
async function setCookie(name: string, data: any) {
  ...
  if (process.env.VERCEL_ENV === "development" || process.env.NODE_ENV === "development") {
    ...
    cookieJar.set(name, data, { httpOnly: true, sameSite: "lax", secure: false, path: '/', });
  } else {
    console.log(`creating secure cookie`)
    cookieJar.set(name, data, { httpOnly: true, sameSite: "strict", secure: true, path: '/', });
  }
...
async function getWebstoreProducts(netoAppURL: string, data: oauthPayload) {
  ...
  try {
    const res = await fetch(`${netoAppURL}${API_ENDPOINT_V2}${data.api_id}/products`, {
      method: "GET",
      headers: {
...
async function getWebstoreProperties(netoAppURL: string, data: oauthPayload) {
  ...
  try {
    const res = await fetch(`${netoAppURL}${API_ENDPOINT_V2}${data.api_id}/properties`, {
      method: "GET",
      headers: {
        Authorization: `${data.token_type} ${data.access_token}`,
        "Content-Type": "application/json",
...
export async function POST(
  ...
  if (request.method === "GET") {
    ...
    try {
      ...
      const idToken = data.id_token;
        ...
        idToken && jwt.verify(idToken, signingKey, {}, function(err, decoded) {
        ...
        accessToken && jwt.verify(accessToken, signingKey, {}, function(err, decoded) {
  ...
  if (process.env.VERCEL_ENV === "development" || process.env.NODE_ENV === "development") {
    console.log(`creating lax cookie`)
    cookieJar.set(`mc_design_auth.${index}`, tokenChunk, { httpOnly: true, sameSite: "lax", secure: false, path: '/', });
  } else {
    console.log(`creating secure cookie`)
    cookieJar.set(`mc_design_auth.${index}`, tokenChunk, { httpOnly: true, sameSite: "strict", secure: true, path: '/', });
  }

  console.log(`creating cookie: mc_design_auth.${index}`)
  console.log(`cookie created: ${cookieJar.has(`mc_design_auth.${index}`)}`)
  ...
  if (process.env.VERCEL_ENV === "development" || process.env.NODE_ENV === "development") {
    console.log(`creating lax cookie`)
    cookieJar.set(`mc_design_auth`, JSON.stringify(encodeAuthCookie), { httpOnly: true, sameSite: "lax", secure: false, path: '/', });
  } else {
    console.log(`creating secure cookie`)
    cookieJar.set(`mc_design_auth`, JSON.stringify(encodeAuthCookie), { httpOnly: true, sameSite: "strict", secure: true, path: '/', });
  }
